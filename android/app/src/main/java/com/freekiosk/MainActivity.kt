...
/**
 * ====================================================================================
 * FK_DEBUG — Instrumentation pack for diagnosing IME / inset / layout jank (#instr)
 * ------------------------------------------------------------------------------------
 * A single, self-contained logging helper. Everything is tagged "FK_DEBUG" so you can
 * isolate the whole trace with:
 *
 *   adb logcat -s FK_DEBUG
 *
 * Every line is prefixed with a millisecond timestamp so events from different
 * callbacks (IME show/hide, focus changes, requestLayout, hideSystemUI, hierarchy
 * dumps) can be correlated on a common timeline.
 *
 * To fully remove the instrumentation: delete this object, the FK_DEBUG.* import
 * usages, and the small hooks marked with "// FK_DEBUG" throughout MainActivity.
 * ====================================================================================
 */
object FKDebug {
  const val TAG = "FK_DEBUG"
  private val ts = SimpleDateFormat("HH:mm:ss.SSS", Locale.US)

  private fun stamp(): String = ts.format(Date())

  fun log(section: String, msg: String) {
    android.util.Log.d(TAG, "[${stamp()}] [$section] $msg")
  }

  fun warn(section: String, msg: String) {
    android.util.Log.w(TAG, "[${stamp()}] [$section] $msg")
  }

  /** Stable, human-readable identity for any View (class + System.identityHashCode + android:id). */
  fun identity(label: String, v: View?): String {
    if (v == null) return "$label=null"
    val idName = try {
      if (v.id != View.NO_ID) v.resources.getResourceEntryName(v.id) else "no-id"
    } catch (e: Exception) { "id#${v.id}" }
    val idHash = Integer.toHexString(System.identityHashCode(v))
    return "$label={cls=${v.javaClass.simpleName} @0x$idHash id=$idName ${v.width}x${v.height} vis=${visStr(v.visibility)}}"
  }
  ...
  /** Find the first descendant of a given class name (e.g. ReactRootView, WebView). */
  fun findByClassName(root: View?, simpleName: String): View? {
    if (root == null) return null
    if (root.javaClass.simpleName == simpleName) return root
    if (root is ViewGroup) {
      for (i in 0 until root.childCount) {
        val hit = findByClassName(root.getChildAt(i), simpleName)
  ...
  /** Find the first WebView descendant. */
  fun findWebView(root: View?): WebView? {
    if (root == null) return null
    if (root is WebView) return root
    if (root is ViewGroup) {
      for (i in 0 until root.childCount) {
        val hit = findWebView(root.getChildAt(i))
        if (hit != null) return hit
      }
    }
    return null
  ...
  /** Recursively dump the view hierarchy with identity + geometry. */
  fun dumpHierarchy(section: String, root: View?, depth: Int = 0) {
    if (root == null) {
      log(section, "hierarchy: <null root>")
      return
    }
    val indent = "  ".repeat(depth)
    val loc = IntArray(2)
    try { root.getLocationOnScreen(loc) } catch (_: Exception) {}
    log(
      section,
      "$indent${root.javaClass.simpleName} @0x${Integer.toHexString(System.identityHashCode(root))} " +
        "id=${safeId(root)} ${root.width}x${root.height} @(${loc[0]},${loc[1]}) " +
        "vis=${visStr(root.visibility)} pad=[${root.paddingLeft},${root.paddingTop},${root.paddingRight},${root.paddingBottom}]"
    )
    if (root is ViewGroup) {
      for (i in 0 until root.childCount) {
        dumpHierarchy(section, root.getChildAt(i), depth + 1)
      }
    }
  }

  private fun safeId(v: View): String = try {
    if (v.id != View.NO_ID) v.resources.getResourceEntryName(v.id) else "no-id"
  } catch (e: Exception) { "id#${v.id}" }
}
...
class MainActivity : ReactActivity() {
  ...
  // FK_DEBUG — last known IME state, used to detect show/hide transitions
  private var lastImeVisible = false
  private var lastImeBottom = -1

  /**
   * FK_DEBUG — Log the identity of the root view, the ReactRootView and the WebView.
   * Recreated views (identity hash changes) are the usual root cause of "insets stop
   * working" bugs, so tracking identity across events is the key signal.
   */
  private fun logViewIdentities(section: String) {
    try {
      val root = findViewById<View>(android.R.id.content)
      FKDebug.log(section, FKDebug.identity("rootView", root))

      val reactRoot = FKDebug.findByClassName(root, "ReactRootView")
        ?: FKDebug.findByClassName(root, "ReactSurfaceView")
      FKDebug.log(section, FKDebug.identity("reactRootView", reactRoot))

      val webView = FKDebug.findWebView(root)
      FKDebug.log(section, FKDebug.identity("webView", webView))
    } catch (e: Exception) {
      FKDebug.warn(section, "logViewIdentities failed: ${e.message}")
    }
  }

  /**
   * FK_DEBUG — Trace requestLayout() + requestApplyInsets() on the content root and its
   * key descendants. Call this whenever you suspect a layout/inset pass is (or is not)
   * being triggered.
   */
  private fun traceRequestLayout(section: String) {
    try {
      val root = findViewById<View>(android.R.id.content)
      FKDebug.log(section, "requestLayout()+requestApplyInsets() on ${FKDebug.identity("rootView", root)}")
      root?.requestLayout()
      root?.let { ViewCompat.requestApplyInsets(it) }

      FKDebug.findWebView(root)?.let { wv ->
        FKDebug.log(section, "requestLayout()+requestApplyInsets() on ${FKDebug.identity("webView", wv)}")
        wv.requestLayout()
        ViewCompat.requestApplyInsets(wv)
      }
    } catch (e: Exception) {
      FKDebug.warn(section, "traceRequestLayout failed: ${e.message}")
    }
  }
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // FK_DEBUG — root/react/webview identity tracking at attach time
    logViewIdentities("onCreate")
    ...
    ViewCompat.setOnApplyWindowInsetsListener(contentView) { view, insets ->
      ...
      // FK_DEBUG — IME visibility + bottom inset logging on every inset pass
      FKDebug.log(
        "INSETS",
        "onApplyWindowInsets: imeVisible=$imeVisible imeBottom=${imeInsets.bottom} " +
          "sysBarsBottom=${sysBars.bottom} currentPadBottom=${view.paddingBottom} " +
          "${FKDebug.identity("target", view)}"
      )

      // Detect an IME transition (show/hide) vs. the previous known state and, on any
      // transition, dump the full hierarchy for correlation.
      if (imeVisible != lastImeVisible || imeInsets.bottom != lastImeBottom) {
        FKDebug.log(
          "IME_TRANSITION",
          "IME state changed: visible ${lastImeVisible}->$imeVisible bottom ${lastImeBottom}->${imeInsets.bottom}"
        )
        lastImeVisible = imeVisible
        lastImeBottom = imeInsets.bottom
        logViewIdentities("IME_TRANSITION")
        FKDebug.dumpHierarchy("IME_TRANSITION", findViewById<View>(android.R.id.content))
      }
  ...
  override fun onWindowFocusChanged(hasFocus: Boolean) {
    ...
    // FK_DEBUG — trace focus changes (correlate with IME show/hide + system bars)
    FKDebug.log("FOCUS", "onWindowFocusChanged: hasFocus=$hasFocus lockTaskPending=$lockTaskPending")
    logViewIdentities("FOCUS")
    if (!hasFocus) {
  ...
  private fun hideSystemUI() {
    // FK_DEBUG — trace every hideSystemUI() invocation with the caller stack
    FKDebug.log(
      "HIDE_SYS_UI",
      "hideSystemUI() called (sdk=${Build.VERSION.SDK_INT}) from: " +
        Thread.currentThread().stackTrace.drop(3).take(4).joinToString(" <- ") { "${it.methodName}:${it.lineNumber}" }
    )
